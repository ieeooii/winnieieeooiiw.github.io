...
| 서비스 | closet-design-system (Brownie) / CLOSET / CLO-SET v3 |
...
총 네 단계에 걸쳐 디자인 시스템을 구축하고 발전시켰다. 1단계(2021.06~2022.08)에서는 중복 컴포넌트로 인한 UX 불일치와 번들 사이즈 문제를 해결하며 번들 사이즈 82% 감소, Lighthouse 95점을 달성했다. 이후 CLOSET 서비스 전용 컴포넌트 추가(2022.03~2022.12), 사내 디자인 시스템 Brownie에 공통 컴포넌트 제안·설계(2022.07~2024.11), CLO-SET v3 디자인 시스템 v2 구축(2023.07~2023.12)까지 전 과정에 기여했다.
...
- **Solve**: Rollup named exports 기반 코드 분할과 sideEffects: false 설정으로 tree-shaking 활성화. ESM 포맷 변환·Rollup externals로 React·Emotion 등 중복 번들링 제거. Emotion.js를 peerDependencies로 전환해 애플리케이션 레벨 단일 인스턴스로 관리.
...
- **Problem**: 컴포넌트 내부 텍스트가 하드코딩되어 다국어 서비스 적용 불가. i18n 라이브러리 직접 내장 시 각 앱의 의존성과 충돌 우려.
