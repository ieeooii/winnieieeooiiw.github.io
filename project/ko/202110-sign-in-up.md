...
| 카테고리 | ETC |
| 서비스 | CLO-SET |
| 기술 스택 | Next.js, TypeScript, MobX (jQuery, Less CSS → React, SCSS 마이그레이션) |
| 개발 기간 | 2021.06 ~ 2021.10 |
| 인원 | 프론트엔드 2, 백엔드 1, 기획자 2, 프로덕트 디자이너 1 (프론트엔드 담당) |
| 서비스 링크 | [style.clo-set.com/account/signin](https://style.clo-set.com/account/signin) |

## 소개

jQuery + Less CSS 기반 레거시 인증 플로우를 React + Emotion.js 기반으로 전면 마이그레이션·리뉴얼했다. Sign In/Up, 비밀번호 find/reset/set/change, OAuth 에러 처리, 재가입 차단, 계정 잠금, Open Redirect 보안 취약점 수정까지 인증 전체 범위를 단독으로 담당했다.
...
### 인증 플로우 Emotion.js + TypeScript 마이그레이션
...
### enum 상태 머신 + @computed로 회원가입 단계·폼 유효성 관리
...
### 다중 인증 수단별 에러 처리와 postMessage Origin 검증으로 SSO 스푸핑 방어
...
### 에러 코드 디스패치 테이블로 로그인 에러·계정 잠금 처리
...
- **Solve**: 에러 코드를 키로 하는 디스패치 테이블로 각 케이스별 모달 내용(제목·본문·버튼 액션)을 매핑. "시도 횟수 초과" 에러는 서버 응답의 남은 잠금 시간 값을 계산해 모달에 동적으로 표시. 계정 잠금 상태는 로그인 스토어의 상태 observable에서 감지하여 모달 트리거 — 비밀번호 변경·이메일 재인증 배너 진입점에서도 동일한 모달 컴포넌트 재사용.
...
### 폼 필드 독립 observer 컴포넌트 분리로 리렌더링 최소화

- **Problem**: 이메일·비밀번호·비밀번호 확인 등 여러 필드가 하나의 폼 컴포넌트 안에 있으면, 어느 한 필드의 상태가 변경될 때 전체 폼이 리렌더링된다. 입력 중 불필요한 리렌더링이 반복되면 타이핑 반응성이 저하될 수 있다.
- **Solve**: 각 폼 필드(이메일·비밀번호·비밀번호 확인)를 MobX `observer`로 감싼 독립 컴포넌트로 분리. 각 컴포넌트가 자신이 구독하는 observable 상태가 변경될 때만 리렌더링되도록 구독 범위를 최소화. Enter 키 입력 시 다음 필드로 자동 포커스 이동하는 ref 체인도 각 컴포넌트 단위로 관리.
- **Result**: 필드별 독립 리렌더링으로 입력 반응성 유지, ref 기반 키보드 탐색으로 폼 접근성 개선
...
마이그레이션 이전의 jQuery 기반 인증 페이지는 BE 레포지토리 안에 포함되어 있었고, 템플릿이 BE 데이터 키에 직접 바인딩되는 구조였다. FE 레포로 분리하면서 BE 의존성이 끊어지고, 컴포넌트 설계·상태 관리·에러 처리 방식을 FE 주도로 결정할 수 있게 되어 훨씬 독립적이고 확장 가능한 구조가 됐다.
