...
| Category | ETC |
| Service | CLO-SET |
| Tech Stack | Next.js, TypeScript, MobX (migrated from jQuery, Less CSS → React, SCSS) |
| Period | 2021.06 – 2021.10 |
| Team | Frontend 2, Backend 1, PM 2, Product Designer 1 (Frontend owner) |
| Service Link | style.clo-set.com/account/signin |

## Overview

Migrated and fully redesigned the jQuery + Less CSS legacy authentication flow to React + Emotion.js. Solely owned the entire authentication scope: Sign In/Up, password find/reset/set/change, OAuth error handling, re-registration blocking, account lock, and Open Redirect security vulnerability fix.
...
Before the migration, the jQuery-based authentication pages lived inside the backend repository, with templates directly bound to backend data keys. Separating into the frontend repository broke the backend dependency, allowing component design, state management, and error handling to be driven entirely by the frontend — resulting in a far more independent and extensible structure.
