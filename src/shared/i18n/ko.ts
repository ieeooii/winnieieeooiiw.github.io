import type { Translations } from './types'

export const ko: Translations = {
  nav: {
    home: '홈',
    projects: '프로젝트',
    about: '소개',
    hireMe: '함께 일해요',
  },
  hero: {
    sentence1: '프론트엔드 엔지니어.',
    sentence2prefix: '문제를 ',
    sentence2suffix: ' 바꿉니다.',
    words: ['구조로', '시스템으로', 'AI 워크플로우로', '솔루션으로', '제품으로', '경험으로'],
    badges: {
      exp: { label: '경력', title: '6+ YRS', sub: 'FRONTEND' },
      proj: { label: '프로젝트', sub: 'COMPLETED' },
      focus: { label: '포커스', title: 'REACT', sub: 'TYPESCRIPT' },
    },
  },
  footer: {
    tagline: '문제를 구조로 바꾸다.\n— AI는 일하는 방법론이다.',
    contact: '연락처',
    copyright: '© 2026 Woojin Lee. All rights reserved.',
  },
  about: {
    pageTitle: 'About',
    headline: '문제를 구조로 바꾸는 프론트엔드 엔지니어',
    bio: '화려한 것보다 잘 정리된 폴더 구조와 아키텍처, 디테일하고 통일성 있는 컴포넌트와 시스템, UI/UX를 보면 마음이 더 즐겁습니다. 같은 문제가 두 번 보이면 코드가 아니라 구조를 의심합니다.\nCLO Virtual Fashion에서 레거시 마이그레이션부터 3D 패션 SaaS까지, 다양한 서비스를 거쳐왔습니다. 기능도 만들지만, 코드 컨벤션 정립부터 디자인 시스템 v1.0~v4.0까지 팀이 같은 문제를 반복하지 않게 하는 일에 더 오래 공을 들였습니다. 일하면서 사용자 경험이 궁금해져 대학원에서 추천 시스템을 연구하기도 했습니다. 연구를 하면서도 결론은 같았습니다. 기술은 비즈니스를 위한 도구이고, 지속 가능한 구조와 좋은 사용자 경험이 곧 서비스의 성장이라고 생각합니다. 빠르게 만드는 법보다 왜 만드는지를 먼저 묻습니다. 그렇게 내린 결정이 사용자에게도 동료에게도 더 오래 도움이 된다고 믿습니다.\n프론트엔드 개발이라는 일을 좋아합니다. 이 일을 오래 하고 싶어서 몇 년째 꾸준히 운동하고 있고, 건강한 몸과 정신이 좋은 코드만큼 중요하다고 생각합니다. 책 읽는 것을 좋아합니다. 스크린의 글은 훑게 되는데, 종이의 글은 곱씹게 되어 종이 쪽을 더 좋아합니다.\n캐치볼을 잘 하는 개발자가 되고 싶습니다. 좋은 협업은 결국 잘 던지고 잘 받는 일이라고 생각합니다. 리뷰든 피드백이든, 상대가 받기 좋게 던지고 온 공은 흘리지 않으려 합니다.',
    sectionExp: 'Experience',
    companyName: 'CLO Virtual Fashion Inc.',
    exp1: {
      role: 'Frontend Engineer',
      period: '2022. 02 – 현재',
      projectName: 'CLO-SET | CLO\'s Next digital fashion Platform',
      desc: 'Hugo Boss, GAP, Bestseller 등 글로벌 패션 회사에서 사용 중인 플랫폼으로, 디자인·개발·생산·리테일·소비로 이어지는 의류 Lifecycle 클라우드 기반 3D 의상 SaaS',
    },
    exp2: {
      role: 'Frontend Developer',
      period: '2020. 11 – 2022. 01',
      projectName: 'CONNECT - The Best 3D Clothing Models',
      desc: '3D 의상 공유, 커뮤니케이션, 판매, 구매 글로벌 디지털 의상 커뮤니티 Open Marketplace | E-Commerce 서비스',
    },
    exp3: {
      role: 'Web Developer',
      period: '2019. 11 – 2020. 10',
      desc: 'NXP(New Experience Platform) Team',
    },
    sectionEdu: 'Education',
    edu1: {
      title: '한양대학교 대학원 공학석사',
      sub: '컴퓨터공학 전공 · GPA 4.17 / 4.5',
      date: '2025년 8월',
    },
    edu2: {
      title: '시각디자인학 전공',
      sub: '전문학사 GPA 4.2 / 4.5 · 학사 GPA 3.89 / 4.5',
      date: '2018년 8월',
    },
    sectionPub: 'Publication',
    pubAbstractLabel: 'Abstract ↓',
    sectionTraining: 'Training / Certification',
    training1: { date: '2019년 8월' },
  },
  projects: {
    viewAll: '전체 프로젝트\n보러가기',
  },
  portfolio: {
    title: 'Projects',
    subtitle: 'Frontend engineering work across product, infrastructure, and developer experience.',
    filterAll: '전체',
    searchPlaceholder: '프로젝트 검색 (제목, 기술 스택, 회사)',
    noResults: '검색 결과가 없습니다.',
    prevProject: '이전 페이지',
    nextProject: '다음 페이지',
    saasImageNotice: '이미지 내 의상은 직접 제작했거나 플랫폼 기본 제공 콘텐츠입니다.',
    notFound: '존재하지 않는 프로젝트입니다.',
    backToList: '프로젝트 목록으로',
  },
}
